...
Helper.prototype.makeDir = function(year, month) {
  const monthStr = (month<10 ? '0'+month : month)+' - '+this.months[month-1];
  const monthDir = path.join(this.OUTPUT_DIR, String(year), monthStr);
  const subDirs = ['csv', 'json', 'map', 'graph', 'html'];

  // Make sub-directories
  for (const dir of subDirs) {
    const subDir = path.join(this.OUTPUT_DIR, String(year), monthStr, dir);
